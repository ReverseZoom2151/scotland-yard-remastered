...
abstract class BaseGameController implements Controller {
	...
	@FXML
	private MenuItem resetViewport;

	@FXML
	private CheckMenuItem focusToggle;
	@FXML
	private CheckMenuItem historyToggle;

	@FXML
	private CheckMenuItem travelLogToggle;
	@FXML
	private CheckMenuItem ticketToggle;
	@FXML
	private CheckMenuItem statusToggle;
	@FXML
	private CheckMenuItem scrollToggle;
	@FXML
	...
	@FXML
	private StackPane roundsPane;
	@FXML
	private StackPane ticketsPane;
	@FXML
	...
	BaseGameController(ResourceManager manager, Stage stage, BoardViewProperty property) {
		...
		map = new MapController(resourceManager, notifications, config);
		...
		var t = new Timeline(1, new KeyFrame(Duration.seconds(1), e -> {
			...
			Window.getWindows().forEach(w -> {
				if (w instanceof Stage) {
					Stage s = (Stage) w;
					if (!s.getTitle().contains(x)) {
						s.setTitle(s.getTitle() + x);
					}
				}
			});
		}));
