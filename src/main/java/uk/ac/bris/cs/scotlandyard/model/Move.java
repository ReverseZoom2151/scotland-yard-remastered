...
/**
 * Represents a move in the ScotlandYard game. A move is an action where a
 * player picks a ticket
...
public interface Move extends Serializable {
	 ...
	 * @param <T>     the return type. Use {@link Void} and return null or your
	 *                custom Unit type for
	 *                side effects.
	 ...
	 *
	 * @param <T> the resulting type; use {@link Void} if not returning (i.e side
	 *            effect)
	 */
	interface Visitor<T> {
		/**
	...
	/**
	 * A visitor adapter that takes two {@link Function}s and turn them into
	 * visitors.
	 *
	...
	final class FunctionalVisitor<T> implements Visitor<T> {
		...
		@Override
		public T visit(SingleMove m) {
			return smf.apply(m);
		}

		@Override
		public T visit(DoubleMove m) {
			return dmf.apply(m);
		}
	}
	...
	final class SingleMove implements Move {
		...
		public Iterable<Ticket> tickets() {
			return ImmutableList.of(ticket);
		}
		...
		@Override
		public int hashCode() {
			return Objects.hash(piece, ticket, destination);
		}
	...
	final class DoubleMove implements Move {
		...
		@Override
		public Iterable<Ticket> tickets() {
			return ImmutableList.of(ticket1, ticket2, DOUBLE);
		}
		...
		@Override
		public String toString() {
			return "x2(" + piece + "@" + source + ", " + ticket1 + ", " + destination1 + ", " + ticket2 + ", "
					+ destination2 + ")";
		}
	}
}
